...
function getTimeSlot(date, useUtc = false) {
...
function getTimeSlotForTimezone(date, timezone) {
    if (!timezone)
...
const pendingProfileRebuilds = new Map();
const activeProfileRebuilds = new Set();
const queuedEvents = [];
...
async function insertEventBatch(batch) {
    const values = [];
    const placeholders = batch.map((event, index) => {
        const offset = index * 9;
        values.push(event.userId, event.eventType, event.trackId || null, event.artistSlug || null, event.genre || null, event.context || null, event.durationListened || 0, event.trackDuration || 0, event.sessionId || null);
...
async function flushEventQueue() {
    if (eventFlushInProgress || queuedEvents.length === 0)
        return;
...
export async function similarTracks(trackId, limit = 10) {
  ...
  `, [trackId]);
    if (coListenedRows.length > 0) {
        const collabIds = coListenedRows.map((row) => row.track_id);
        const placeholders = collabIds.map((_, i) => `$${i + 1}`).join(',');
        candidateQueries.push(query(`SELECT * FROM tracks WHERE status = 'ready' AND id IN (${placeholders})`, collabIds));
    }
    ...
    const maxSharedUsers = coListenedRows.reduce((max, row) => Math.max(max, Number(row.shared_users || 0)), 0);
    const maxEngagementScore = coListenedRows.reduce((max, row) => Math.max(max, Number(row.engagement_score || 0)), 0);
...
export async function continueListening(userId, limit = 10) {
  ...
  `, [userId]);
    if (recent.length === 0)
        return [];
    const unfinished = recent.filter((row) => {
    ...
    if (unfinished.length === 0)
        return [];
    const trackIds = unfinished.map((r) => r.track_id);
    ...
    const recentMap = new Map(unfinished.map((r) => [r.track_id, r]));
    tracks.sort((a, b) => {
