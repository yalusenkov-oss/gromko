...
function resolveSpotiflacDir() {
    ...
    for (const dir of candidates) {
        ...
        if (fs.existsSync(cmdServer) || fs.existsSync(binServer))
            return dir;
    }
    return null;
}
 ...
 */
function findGoBin(SPOTIFLAC_DIR) {
    // System Go
    ...
    if (process.env.SPOTIFLAC_AUTO_INSTALL_GO === '0')
        return null;
    ...
    if (!goArch)
        return null;
    if (goRuntimeBootstrapAttempted)
        return null;
 ...
 */
function buildSpotiflacBinary(SPOTIFLAC_DIR) {
    ...
    const result = spawnSync(goBin, ['build', '-trimpath', '-ldflags=-s -w', '-o', outBin, './cmd/server'], { cwd: SPOTIFLAC_DIR, env: buildEnv, stdio: ['ignore', 'pipe', 'pipe'], timeout: 300_000 });
...
function startSpotiflac() {
    ...
    fetch(`http://localhost:${spotiflacPort}/health`)
        ...
        spotiflacProcess.stdout?.on('data', (data) => {
            const msg = data.toString().trim();
            if (msg)
                console.log('  [SpotiFLAC]', msg);
        });
        spotiflacProcess.stderr?.on('data', (data) => {
            const msg = data.toString().trim();
            if (msg)
                console.error('  [SpotiFLAC ERR]', msg);
        });
...
async function recalcArtistStats() {
    try {
        ...
        const combinedArtists = await query(`SELECT id, name, slug, genre
       FROM artists
...
async function backfillPlayHistory() {
    try {
        ...
        const orphanCount = await queryOne(`SELECT COUNT(*) as c FROM play_history WHERE user_id IS NULL`);
