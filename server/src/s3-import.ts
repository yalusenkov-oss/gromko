...
import { query, queryOne, execute, initSchema, closeDb, getPool } from './db.js';
...
function normalizeGenre(raw?: string): string | undefined {
  ...
  // Cached
  if (GENRE_MAP[trimmed] !== undefined) return GENRE_MAP[trimmed] || undefined;
...
async function processPool(tracks: S3Track[], existingFiles: Set<string>, existingTracks: Set<string>) {
  let cursor = 0;
...
async function main() {
  ...
  const existingTracks = new Set<string>(
    existingTrackRows.map((r: any) => `${r.artist}:::${r.title}`).filter((k: string) => k !== ':::')
  );
