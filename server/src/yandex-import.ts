...
const jobs = new Map<string, YandexImportJob>();
const artistGenreCache = new Map<string, string | null>();

function getYmdDir(): string {
  const candidates = [
...
function yandexToken(): string {
  return (process.env.YANDEX_MUSIC_TOKEN || '').trim();
}

function runCommand(cmd: string, args: string[], cwd: string, envExtra?: Record<string, string>, timeoutMs = 60_000): Promise<void> {
...
async function ensureYmdEnv(ymdDir: string): Promise<string> {
...
function collectAudioFiles(dir: string): string[] {
  ...
  for (const entry of fs.readdirSync(dir, { withFileTypes: true })) {
    ...
    if (AUDIO_EXTS.has(ext)) result.push(full);
  }
  return result;
}

function findCoverNearAudio(audioPath: string): string | null {
...
function detectJobType(url: string): YandexImportJob['type'] {
...
function normalizeArtistDisplay(artistRaw: string): string {
  return artistRaw
    .replace(/\s*;\s*/g, ', ')
    .replace(/\s*,\s*/g, ', ')
    .replace(/\s{2,}/g, ' ')
...
function buildYandexImportComment(sourceUrl: string, meta: {
  title: string;
  artist: string;
  album: string | null;
  year: number;
...
function isYandexMusicUrl(url: string): boolean {
...
async function findArtistGenre(artist: string): Promise<string | null> {
  ...
  try {
    const row = await queryOne<{ genre?: string }>('SELECT genre FROM artists WHERE slug = $1', [slug]);
...
async function resolveYandexGenre(metaGenre: string | undefined, requestedGenre: string, artist: string): Promise<string> {
  if (metaGenre && metaGenre.trim()) return metaGenre.trim();
  const fromArtist = await findArtistGenre(artist);
  if (fromArtist) return fromArtist;
  return (requestedGenre || 'Другое').trim();
}

async function ensureArtists(trackId: string, artistRaw: string, genre: string): Promise<string> {
  ...
  for (let i = 0; i < names.length; i++) {
    ...
    if (!artistId) {
      ...
      await execute(
        ...
        [artistId, name, slug, genre || 'Другое'],
      );
    }
...
function finalizeJob(job: YandexImportJob) {
  job.progress = 100;
...
async function runYandexCli(job: YandexImportJob): Promise<string[]> {
...
async function runImport(job: YandexImportJob, genre: string) {
  try {
...
async function runSubmission(job: YandexImportJob, userId: string, genre: string) {
  try {
    const files = await runYandexCli(job);
    job.totalTracks = files.length;
    job.tracks = files.map((f) => ({
...
export function getYandexJob(id: string): YandexImportJob | undefined {
  return jobs.get(id);
}

export async function checkYandexImportHealth(): Promise<{ available: boolean; reason?: string }> {
  try {
    const ymdDir = getYmdDir();
    if (!fs.existsSync(ymdDir)) return { available: false, reason: 'Каталог yandex-music-downloader-main не найден' };
    if (!yandexToken()) return { available: false, reason: 'Не задан YANDEX_MUSIC_TOKEN' };
    const py = await ensureYmdEnv(ymdDir);
...
export function startYandexSubmission(sourceUrl: string, userId: string, isAdmin: boolean, genre = 'Другое'): string {
  const url = normalizeYandexUrl(sourceUrl);
  if (!isYandexMusicUrl(url)) {
    throw new Error('Только ссылки Yandex Music (music.yandex.ru)');
  }

  const job: YandexImportJob = {
    id: uuid(),
    sourceUrl: url,
  ...
  if (isAdmin) {
    runImport(job, genre).catch((err: any) => {
      job.status = 'error';
      job.error = err.message;
      job.finishedAt = new Date().toISOString();
    });
  } else {
    runSubmission(job, userId, genre).catch((err: any) => {
      job.status = 'error';
      job.error = err.message;
      job.finishedAt = new Date().toISOString();
    });
