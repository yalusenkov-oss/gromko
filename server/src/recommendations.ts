...
export async function rebuildTasteProfile(userId: string): Promise<void> {
  ...
  for (const ev of events) {
    ...
    // Skip/play counting
    if (ev.event_type === 'skip') skipCount++;
...
function scoreTrack(
  ...
  if (!profile) return track.plays * 0.001; // fallback: popularity
...
export async function nextTrack(
  ...
  const scored = all.map((t: any) => {
    ...
    if (t.genre === current.genre) sim += 0.35;

    // Same artist bonus
    if (t.artist_slug === current.artist_slug) sim += 0.15;
...
export async function similarArtists(artistSlug: string, limit = 6): Promise<any[]> {
  ...
  const slugs = coListened.map((r: any) => r.artist_slug).slice(0, limit);
